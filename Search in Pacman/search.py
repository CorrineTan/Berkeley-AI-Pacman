...
def backTrace(endPoint, routes):
    path = []
    current = endPoint
    while current in routes:
        path = [routes[current][1]] + path
...
def uniformCostSearch(problem):
    ...
    while not priorityQueue.isEmpty():
        node, route = priorityQueue.pop()
        if problem.isGoalState(node):
            return route
        if node not in visited:
            for nextState, action, cost in problem.getSuccessors(node):
                if nextState not in visited:
                    newAction = route + [action]
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    visited = []
    priorityQueue = util.PriorityQueue()
    priorityQueue.push((problem.getStartState(), []), nullHeuristic(problem.getStartState(), problem))
    while not priorityQueue.isEmpty():
        node, route = priorityQueue.pop()
        if problem.isGoalState(node):
            return route
